...
A Ubiquiti-style per-link quality card, one compact row per WAN interface: a brand-colored ISP monogram badge, current status, rolling 24h uptime %, how long it's been in its current up/down state, average latency, and a 24-hour spike/downtime bar graph beneath — bar height tracks latency, full-height red bars mark outages, hoverable for exact numbers. Works on both `mwan3`-managed multi-WAN routers (one row per link) and plain single-WAN routers (one row, no `mwan3` needed at all). ISP badges are simple brand-colored monograms rather than downloaded logo images — there are no image assets in this codebase, and it sidesteps reproducing anyone's actual trademarked logo file; unrecognized ISPs fall back to a neutral badge with their name's first letter.

This is backed by a small always-running background collector (`hwdash-wanmon`, a separate `procd`-managed service, not the on-demand `rpcd` call everything else in this app uses) that pings a fixed IP anchor (never a domain — zero DNS load added anywhere) through each WAN roughly every 15 seconds and keeps a rolling 24h ring buffer in `/tmp` (tmpfs, so it costs RAM, not flash, and clears on reboot). The dashboard's `info` call only *reads* that buffer and aggregates it into 96 15-minute buckets — it never pings anything itself, so having the dashboard open doesn't add any extra probing beyond what the collector already does on its own fixed schedule.

The interesting part is *how* it pings a specific non-default WAN correctly on a multi-WAN router. `mwan3`'s policy routing is driven by an nftables fwmark that's only applied to traffic matching one of its own rules — a plain `ping -I <device>` or `curl --interface <device>` from an arbitrary process silently fails or, worse, silently routes out the *wrong* interface, since the kernel falls back to the default route once the fwmark isn't set. `mwan3` solves this for its own tracker (`mwan3track`) via an `LD_PRELOAD` shim (`libwrap_mwan3_sockopt.so`) that sets the correct fwmark on the socket before routing happens; `hwdash-wanmon` reuses that exact same library rather than reinventing it, reading each interface's real fwmark straight out of the live `nftables` ruleset. ISP names are resolved the same way — via a DNS-based ASN lookup (Team Cymru's `origin.asn.cymru.com`) against each WAN's real public egress IP, correctly reached even when that WAN is a DHCP client behind another private router (a second physical hop, not just CGNAT) rather than the CGNAT/PPPoE-peer address alone. On a plain single-WAN router there's no fwmark to look up at all — the wrapper library is skipped entirely and a normal device-bound `ping`/`curl` is used directly, since there's no ambiguity about which interface to use. Both the ping loop and the ISP lookup only ever consider WAN-zone interfaces that currently have a live default route — a zone member with no gateway (a stale or decoy config) is correctly excluded rather than showing up as a permanently-dead WAN.
