...
**NVMe Details** shows identity (model, serial, firmware, transport) from sysfs, plus a full SMART health readout when the optional `nvme-cli` package is installed: a Healthy/Warning/Critical badge, Wear (Percentage Used) and Available Spare bars, Temperature, Power-On Hours, Power Cycles, Unsafe Shutdowns, Media Errors, total Data Read/Written, and decoded critical-warning flags (spare below threshold, temperature threshold, reliability degraded, media read-only, backup device failed) — parsed from `nvme smart-log -o json`. `nvme-cli` isn't installed by default (there's nothing to read it on boards without an NVMe drive), so the SMART fields are simply absent without it; identity still shows from sysfs alone. Cached for 30 seconds since SMART counters change slowly.
