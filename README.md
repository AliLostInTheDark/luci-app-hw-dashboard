...
On `mwan3`-managed multi-WAN routers, a Ubiquiti-style per-link quality card: one row per WAN interface showing its ISP name, current status, rolling uptime %, average latency, and a 24-hour spike/downtime timeline bar (each segment colored by loss and latency, hoverable for exact numbers). Absent entirely on single-WAN or non-`mwan3` routers — there's nothing to differentiate.
