...
return view.extend({
    ...
    render: function() {
        ...
        Object.keys(cardRegistry).forEach(function(key) {
            ...
            //
            // Ghost-clone technique (the same one SortableJS's own
            // touch/mouse fallback uses, not native HTML5 drag-and-drop --
            // that has no real touch support): a detached, position:fixed
            // copy follows the raw pointer coordinates, while the real card
            // stays in the grid (dimmed) as the thing that actually gets
            // reordered via insertBefore. This is strictly more robust than
            // moving/transforming the live card directly -- the ghost is
            // permanently pointer-events:none, so elementFromPoint below
            // can never hit-test into its own dragged element by accident,
            // with no toggle-and-restore timing to get wrong.
            handle.addEventListener('pointerdown', function(ev) {
                ev.preventDefault();
                try { handle.setPointerCapture(ev.pointerId); } catch (e) {}

                var startRect = n.getBoundingClientRect();
                var offsetX = ev.clientX - startRect.left;
                var offsetY = ev.clientY - startRect.top;

                var ghost = n.cloneNode(true);
                ghost.removeAttribute('id');
                var idEls = ghost.querySelectorAll('[id]');
                for (var gi = 0; gi < idEls.length; gi++) idEls[gi].removeAttribute('id');
                var ghostBar = ghost.querySelector('.hw-card-editbar');
                ...
                ghost.style.boxShadow = '0 16px 34px rgba(0,0,0,0.4)';
                ghost.style.transition = 'none';
                document.body.appendChild(ghost);
                n.classList.add('hw-card-placeholder');
