    ...
    render: function() {
        ...
        Object.keys(cardRegistry).forEach(function(key) {
            ...
            handle.addEventListener('pointerdown', function(ev) {
                ...
                var lastOver = null;
                // Relative to this, not fixed for the whole gesture: reset
                // every time the card's underlying grid slot actually moves
                // (below), so the translate offset keeps tracking the
                // pointer smoothly instead of jumping by however far the
                // reflow just shifted the card's own layout box.
                var startX = ev.clientX, startY = ev.clientY;
                var onMove = function(mv) {
                    // Without this the dragged card just sat in place,
                    // dimmed, until the pointer physically crossed a
                    // neighbor's midpoint -- no visible feedback for a
                    // modest drag, which read as "nothing is happening".
                    n.style.transform = 'translate(' + (mv.clientX - startX) + 'px, ' + (mv.clientY - startY) + 'px)';
                    // elementFromPoint hit-tests the painted frame, and the
                    // translated card now visually covers whatever is
                    // underneath it -- without this it would just find
                    // itself every time and never detect another card to
                    // swap with.
                    var prevPE = n.style.pointerEvents;
                    n.style.pointerEvents = 'none';
                    var el = document.elementFromPoint(mv.clientX, mv.clientY);
                    n.style.pointerEvents = prevPE;
                    var card = el;
