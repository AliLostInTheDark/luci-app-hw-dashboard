function add(list, b, ch) {
	list[b] = list[b] (list[b] == "" ? "" : ",") ch
}
...
in_freq {
	...
	freq = 0
	# Newer iw prints "5180.0 MHz".
	if (match($0, /[0-9]+(\.[0-9]+)? MHz/)) freq = substr($0, RSTART, RLENGTH - 4) + 0
	if (ch != "" && freq > 0) {
		...
		bands[temp_band] = 1
		if ($0 ~ /disabled/) {
			add(disabled, temp_band, ch)
		} else {
			add(enabled, temp_band, ch)
			if ($0 ~ /no IR/ || $0 ~ /radar/) add(exceptions, temp_band, ch)
		}
	}
}
/Capabilities:/ || /^[a-zA-Z]/ { in_freq = 0 }
...
# "HT TX/RX MCS rate indexes supported: 0-31" -> 4 streams
